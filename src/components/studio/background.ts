// Spatial background: parallax dot-field + noise-displaced "home star" orb that
// tracks the brand node. Ported from the design prototype (js/labs-bg.js).
...
export function initBackground(
  ...
  // 3 parallax dot layers
  const layers: THREE.Points[] = [];
  function makeLayer(count: number, spread: number, size: number, opacity: number, depth: number) {
  ...
  // home-star orb (noise-displaced shader)
  const vert = `
  ...
  function frame() {
    ...
    for (const lyr of layers) {
      const d = lyr.userData.depth as number;
